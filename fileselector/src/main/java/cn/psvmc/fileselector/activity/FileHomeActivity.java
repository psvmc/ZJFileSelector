package cn.psvmc.fileselector.activity;
...
import cn.psvmc.fileselector.bean.SelectedFiles;
import cn.psvmc.fileselector.util.SDCardScanner;
...
public class FileHomeActivity extends Activity implements OnClickListener {
	...
	public void onClick(View view) {
		if(view.getId() == R.id.tv_file_home_cancel){
			onBackPressed();
		}else if(view.getId() == R.id.rl_file_home_my){
			FileMyActivity.actionStart(instance, myFolderPath, REQUEST);
		}else if(view.getId() == R.id.rl_file_home_mm){
			FileFolderActivity.actionStart(instance, Environment.getRootDirectory().getParent(), REQUEST);
		}else if(view.getId() == R.id.rl_file_home_big){
			if (externalPaths != null && externalPaths.size() > 0) {
		...
		}else if(view.getId() == R.id.rl_file_home_sd){
			if (externalPaths != null && externalPaths.size() > 1) {
